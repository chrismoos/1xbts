...
/// Mobile-station side processor that decodes Sync Channel messages from
...
pub struct MobileStationProcessor {
    ...
    /// Chip-start value for the next frame to be emitted.
    next_chip: usize,
...
impl PipelineProcessor for MobileStationProcessor {
    /*fn process_block(&mut self, block: SampleBlock) -> Vec<SampleBlock> {
        ...
        eprintln!(
            "mobile_station_bits chip_start={} len={} bits={},pn_phase={}",
            block.chip_start,
            block_bits.len(),
            bit_string,
            pn_phase
...
mod tests {
    ...
    use super::MobileStationProcessor;
    use crate::{
        lac::crc30,
    ...
    fn test_mobile_station_processor_emits_sync_event() {
        let mut p = MobileStationProcessor::new().with_self_alignment(true);
    ...
    fn test_mobile_station_processor_no_lock_on_noise() {
        let mut p = MobileStationProcessor::new();
