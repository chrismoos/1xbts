...
extern void pit_shrp (float *x,	/* in/out: impulse response (or algebraic code) */
		      int pit_lag,	/* input : pitch lag                            */
		      float sharp,	/* input : pitch sharpening factor              */
		      int L_subfr	/* input : subframe size                        */
    );

extern void corr_xh (float *x,	/* input : target signal                                   */
...
extern void cod8_35 (float dn[],	/* input : correlation between target and h[]            */
...
extern void cod3_10 (float dn[],	/* input : correlation between target and h[]            */
		     float cn[],	/* input : residual after long term prediction           */
		     int l_subfr,	/* input : lenght of subframe                            */
		     float h[],	/* input : impulse response of weighted synthesis filter */
		     float cod[],	/* output: algebraic (fixed) codebook excitation         */
...
extern void dec3_10 (int index,	/* input: indices of 3 pulses (10 bits, 1 word)        */
		     float cod[]	/* output: algebraic (fixed) codebook excitation       */
    );

extern void dec8_35 (int *indx,	/* input: indx of 8 pulses (8+8+8+11 = 35 bits)    */
		     float cod[]	/* output: algebraic (fixed) codebook excitation      */
    );


/* Factorial Pulse Codebook */
...
/* Prototypes */
void cod_fpmp (float *xw, float *x, int l_subfr, float *h, float *ck,
	       float y[], float *gain, int *indx);
...
#include "fplib_def.h"
intVar factorial_pack_new (int Tlen, float *vector);
int factorial_unpack_new (intVar code, int len, int Np, int *retvector);
#endif
