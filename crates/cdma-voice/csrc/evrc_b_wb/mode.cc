...
static char const rcsid[] =
    "$Id: mode.cc,v 1.10.6.3 2007/06/24 06:43:15 apadmana Exp $";
...
FGV_MEM::update_average_rate (int rate)	//valid rates are 1,2,3,4
{
	...
	if (args->avg_rate_control) {
		...
		if (numactive == args->ratewin) {
		    ...
		    if (ave_rate_kbps == 0)
			ave_rate_kbps =
			    (long int) (args->ratewin *
					args->avg_rate_target * 10);
		    long int avratetarg =
			(long int) ((args->ratewin * 10) * (2 - 0.02) *
				    args->avg_rate_target - ave_rate_kbps);
		    pattern_m =
			(int) (1000 *
			       (avratetarg -
				7.5 * args->ratewin * 10) / (10 *
							      args->ratewin *
							      (9.0 - 7.5)));
		...
		if (numactive == args->ratewin) {
			...
			ave_rate_kbps =
			    (long int) (args->ratewin *
					args->avg_rate_target * 10);
		    long int avratetarg =
			(long int) ((args->ratewin * 10) * (2 - 0.00) *
				    args->avg_rate_target - ave_rate_kbps);
		    pattern_m =
			(int) (1000 *
			       (avratetarg -
				6.6 * args->ratewin * 10) / (10 *
							      args->ratewin *
							      (7.5 - 6.6)));
